          ...
          <div id="scoreboard-table">
            <form action="score.jsp" method="POST">
              <%
                ...
                String home_team = request.getParameter("teamA");
                String visitor_team = request.getParameter("teamB");

                if (home_team == null || visitor_team == null) {

              %>
              <script type="text/javascript">
                alert("Please enter the team name.");
                window.location.href = "enter_team_name.html";
              </script>
              <%}%>
              <figure class="table">
                <table class="ck-table-resized">
                  ...
                  <tr>
                    ...
                    <td style="width:10%;">
                      <label class="team_name"> <input type="text" style="color:#4e0101;visibility: visible;opacity: 1;width:80px; border: none;text-align: center; font-size: 24px; font-weight: bold" id="teamAName" name="teamA" autocomplete="off" value="<%=request.getParameter("teamA")%>"></label>
                    </td>
                    ...
                    <td style="width:10%;">
                      <label class="team_name"> <input type="text" style="color:#4e0101;visibility: visible;opacity: 1;width:80px; border: none;text-align: center; font-size: 24px; font-weight: bold" id="teamBName" name="teamB" autocomplete="off" value="<%=request.getParameter("teamB")%>"></label>
                    </td>
                    <td style="width:35%;">
                  ...
                  <tr>
                    <td><button type="button" class="add-foul" data-team="A">Add Foul</button>
                      <button type="button" class="remove-foul" data-team="A">Remove Foul</button></td>
                    <td><span id="teamAFouls">0</span></td>
                    <td>&nbsp;</td>
                    <td>&nbsp;</td>
                    <td><span id="teamBFouls">0</span></td>
                    <td><button type="button" class="add-foul" data-team="B">Add Foul</button>
                      <button type="button" class="remove-foul" data-team="B">Remove Foul</button>
                    </td>
                  </tr>
                  <tr class="tr-title">
                    <td colspan="6">
                      QUARTER: <span id="currentQuarter" style="color:#371804; font-weight: bold; font-size: larger;text-shadow: none;">1st</span>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="2">
                    ...
                    <td colspan="2" class="digit">
                      <span id="quarterTimer">10:00</span>
                    </td>
                    <td colspan="2">
                  ...
                  <tr>
                    ...
                    <td class="digit" colspan="2">
                      00:<span id="shotClock">24</span>
                    </td>
                    <td colspan="2">
                  ...
                  <tr>
                    <td>
                      ...
                      <button type="button" class="timeout-btn" data-team="A" data-seconds="60">1min</button><hr/>
                      <button type="button" id="stopATimeout">Stop</button>
                      <button type="button" id="resetATimeout">Reset</button>
                    </td>
                    <td>
                      Home
                    </td>
                    ...
                    <td><button type="button" class="timeout-btn" data-team="B" data-seconds="30">30s</button>
                      <button type="button" class="timeout-btn" data-team="B" data-seconds="60">1min</button><hr/>
                      <button type="button" id="stopBTimeout">Stop</button>
                      <button type="button" id="resetBTimeout">Reset</button>
                    </td>
                  </tr>
                  <tr>
                    <td colspan="6">
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    let quarterTime = 600;
    let shotClock = 24;
    let quarterInterval, shotClockInterval= {};
    // Global object to store timeout intervals
    ...
    // Function to stop Team A's timeout
    function stopATimeout() {
      if (timeoutIntervals.A !== null) {
        clearInterval(timeoutIntervals.A);
      }
    }

// Function to reset Team A's timeout
    function resetATimeout() {
      timeoutIntervals.A = null;
      document.getElementById('teamATimeoutTimer').innerText = '0';
    }

// Function to stop Team B's timeout
    function stopBTimeout() {
      if (timeoutIntervals.B !== null) {
        clearInterval(timeoutIntervals.B);
      }
    }

// Function to reset Team B's timeout
    function resetBTimeout() {
      timeoutIntervals.B = null;
      document.getElementById('teamBTimeoutTimer').innerText = '0';
    }
    ...
    // ✅ Attach event listeners for fouls
    document.querySelectorAll(".remove-foul").forEach(button => {
      button.addEventListener("click", function () {
        let team = this.dataset.team;
        removeFoul(team);
      });
    ...
    document.getElementById('stopATimeout').addEventListener('click', stopATimeout);
    document.getElementById('resetATimeout').addEventListener('click', resetATimeout);
    document.getElementById('stopBTimeout').addEventListener('click', stopBTimeout);
    document.getElementById('resetBTimeout').addEventListener('click', resetBTimeout);
  });
  ...
  document.getElementById("save_button").addEventListener("click", function (event) {
    ...
    fetch("http://localhost:8080/SimpleWebApp_war_exploded/ProcessGameServlet", {
      ...
      if (response.redirected) {
        window.location.href = response.url; // ✅ Follows redirect to viewHistory.jsp
      } else {
